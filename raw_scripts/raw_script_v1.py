    ...
    for c in cnts:
        ...
        if higherThanHeight:
            if w > boxWidthTresh and h > boxHeightTresh:
                ...
                fileName = folder + "/" + outputImagePrefix + "_" + str(index) + ".png"
...
def createImagesFromImageBoxes(img, originalName, savePreprocessingImages=False):
    contours = getBoxesContours(img, originalName, savePreprocessingImages)
...
def getHeader():
    ...
    header = InvoiceHeader(
        business_name=getHeaderConcept(img_file="processing/header_concept_2_1.png"),
        business_address=getHeaderConcept(img_file="processing/header_concept_2_2.png"),
        vat_condition=getHeaderConcept(img_file="processing/header_concept_2_3.png"),
        document_type=getHeaderConcept(img_file="processing/header_concept_1_1.png"),
        document_number=getHeaderConcept(img_file="processing/header_concept_1_2.png"),
        checkout_aisle_number=getHeaderConcept(
            img_file="processing/header_concept_1_3.png"
        ),
        issue_date=getHeaderConcept(img_file="processing/header_concept_1_4.png"),
        seller_cuit=getHeaderConcept(img_file="processing/header_concept_1_5.png"),
        gross_income=getHeaderConcept(img_file="processing/header_concept_1_6.png"),
        business_opening_date=getHeaderConcept(
            img_file="processing/header_concept_1_7.png"
        ),
        client_cuit=getHeaderConcept(img_file="processing/header_concept_4_2.png"),
        client_name=getHeaderConcept(img_file="processing/header_concept_4_1.png"),
        client_address=getHeaderConcept(img_file="processing/header_concept_4_3.png"),
        client_vat_condition=getHeaderConcept(
            img_file="processing/header_concept_4_4.png"
        ),
        sale_method=getHeaderConcept(img_file="processing/header_concept_4_5.png"),
    )
...
def getFooter(invoice_type):
    ...
    if should_paint:
        ...
        contours = getBoxesContours(image, "test", False)

        for c in contours:
            x, y, w, h = cv2.boundingRect(c)
            if w < image.shape[1]:
                cv2.rectangle(
                    image, (x - 5, y - 5), (x + w + 3, y + h + 3), (255, 255, 255), -1
                )
                cv2.imwrite("temp/footer_box_1.png", image)
